...
const roomsData = [
  { token: tokens[0].replace(/^"+|"+$/g, ""), lastRefresh: Date.now() },
  { token: tokens[1].replace(/^"+|"+$/g, ""), lastRefresh: Date.now() }
];

// Token'ı Room.create'a tırnaksız ver
const tokenRoom1 = roomsData[0].token;
const tokenRoom2 = roomsData[1].token;


console.log("Alınan token 1:", tokenRoom1);
console.log("Alınan token 2:", tokenRoom2);
...
const room1 = Room.create({
  ...
  onOpen: (room) => {
      ...
      console.log("Room1 opened!");

      const roomIndex = 0; 

   console.log("Alınan Token (room1):", roomsData[roomIndex].token);
logError("Room opened! (room1)");
logError("Alınan token:", roomsData[roomIndex].token);

    room.fakeSetTeamsLock(true);


     // room1
async function refreshRoomTokenPeriodically() {
  const ONE_HOUR = 60*60*1000;

  if (!roomsData[roomIndex].token || Date.now() - roomsData[roomIndex].lastRefresh > ONE_HOUR) {
    try {
      const res = await refreshRoomToken({ token: roomsData[roomIndex].token });
      roomsData[roomIndex].token = res.token;
      roomsData[roomIndex].lastRefresh = Date.now();
      console.log("Room 1 token yenilendi:", roomsData[roomIndex].token);
    } catch (err) {
      console.error("Token invalid! rcr gerekebilir", err);
    }
  }
}
setInterval(refreshRoomTokenPeriodically, 5*60*1000);
    ...
    */
    ...
    function logTurnQueue() {
      ...
      logGame(`🔹 turnQueue: ${turnQueue.map(id => room.getPlayer(id)?.name || id).join(", ")}`);
    }
    ...
    room.onOperationReceived = (type, msg, globalFrameNo, clientFrameNo, customData) => {
      ...
      if (command === "!delik") {
        ...
        if (!pocketIndexStr) {
          ...
          room.sendAnnouncement(`Delikler: 1: Sol Üst, 2: Orta Üst, 3: Sağ Üst, 4: Sağ Alt, 5: Orta Alt, 6: Sol Alt`, player.id, 0xFFFF00);
          return false;
        }
...
Rating (Puan): ${ratingDisplay}`.trim();
    ...
    room.onPlayerJoin = (player) => {
      ...
      logGame(`Pending AFK: ${Array.from(pendingAfk).join(", ")}`);
      ...
      if (gameRunning && turnQueue.length >= 2) {
        console.log("join check:", { gameRunning, length: turnQueue.length });
        logGame(`join check: ${gameRunning}, length: ${turnQueue.length}`);
    ...
    function restartGame() {
      ...
      logGame(`Process pendingden önce: ${turnQueue.map(id => room.getPlayer(id)?.name || id).join(", ")}`);

      processPendingAfk();

      console.log("Process pendingden sonra: ", turnQueue.map(id => room.getPlayer(id)?.name || id));
      logGame(`Process pendingden sonra: ${turnQueue.map(id => room.getPlayer(id)?.name || id).join(", ")}`);
      ...
      playersToCheck.forEach(playerId => {
        ...
        if (playerScores[playerId] === TOTAL_COLOR_BALLS) {
          ...
          if (!BLACK_BALL_TARGETS.has(playerId)) {
            ...
            room.sendAnnouncement(`1: Sol Üst, 2: Orta Üst, 3: Sağ Üst, 4: Sağ Alt, 5: Orta Alt, 6: Sol Alt`, player.id, 0x00AAFF)
    ...
    room.onPlayerLeave = (player) => {
      ...
      if (wasCurrentPair) {
        ...
        logGame(`Already Ranked çıktıdan önce: ${alreadyRanked}`);
    ...
    room.onPlayerBallKick = (playerId) => {
      ...
      logGame(`=== BallKick Başladı ===`);
      logGame(`Player: ${player.name} (${player.id})`);
      logGame(`CurrentPlayer: ${currentPlayer}`);
      logGame(`GameRunning: ${gameRunning}`);
      logGame(`TurnQueue: ${turnQueue.map(id => room.getPlayer(id)?.name || id).join(", ")}`);
    ...
    function checkDiscGoal(disc, index) {
      ...
      for (let i = 0; i < pocketCenters.length; i++) {
        ...
        if (dist < EFFECTIVE_HOLE_RADIUS) {
          ...
          logGame(`Top girdi: ${color} (ID: ${index}) by ${lastPlayer.name} (${lastPlayer.id})`);
          ...
          if (!playerColors[lastPlayer.id] && color !== "beyaz" && color !== "siyah") {
            ...
            logGame(`Renk atandı: ${lastPlayer.name} -> ${color}, Rakip -> ${playerColors[opponentId]}`);
            ...
            logGame(`Doğru top sokuldu: ${color} by ${lastPlayer.name}. Skor: ${playerScores[lastPlayer.id]}`);
          }

          // ⚠️ Rakibin topunu soktuysa işaretle
          const opponentId = turnQueue.find(id => id !== lastPlayer.id);
          console.log(`Önce : playerScores[opponentId]:  ${playerScores[opponentId]}`);
          logGame(`Rakip top kontrolü: ${color} by ${lastPlayer.name}`);
          if (color !== "beyaz" && playerColors[lastPlayer.id] && playerColors[lastPlayer.id] !== color && color !== "siyah") {
            ...
            if (opponentColor === color) {
              ...
              logGame(`Rakip top sokuldu: ${color} by ${lastPlayer.name}. Rakip Skor: ${playerScores[opponentId]}`);
          ...
          if (color === "siyah") {
            ...
            if (!ownColor || !playerScores[lastPlayer.id]) {
              ...
              black_whiteballcheck = setTimeout(() => {
                ...
                logGame(`[DEBUG] Siyah Top Giriş Deliği İndeksi: ${correctPocketIndex !== -1 ? POCKET_NAMES[correctPocketIndex] : 'Bilinmiyor'}`);
                ...
                logGame(`blackBallDisc bulundu mu?: ${!!blackBallDisc}`); // true veya false dönmeli
            ...
            logGame(`Already Ranked durumu: ${alreadyRanked}`);
            ...
            if (!controlIsDone) {
              ...
              logGame(`Already Ranked durumu (timeout sonrası): ${alreadyRanked}`);
            ...
            logGame(`Already Ranked durumu (restart sonrası): ${alreadyRanked}`);
...
const room2 = Room.create({
  ...
  onOpen: (room) => {
const roomIndex = 1; // room2

    console.log("Room2 opened!");
    console.log("Alınan Token (room2):", roomsData[roomIndex].token);
logError("Room opened! (room1)");
logError("Alınan token:", roomsData[roomIndex].token);
    room.fakeSetTeamsLock(true);
